  ...
  return (
    <nav className="sticky top-0 w-full z-50 bg-slate-50/90 dark:bg-[#111111]/90 backdrop-blur-md border-b border-slate-200 dark:border-white/5 flex justify-between items-center px-4 md:px-8 py-4 transition-colors duration-300">
      ...
      <div className="hidden lg:flex gap-8 font-mono text-sm tracking-widest uppercase">
        <a
          href="#experience"
          className="text-slate-900 dark:text-white border-b-2 border-cyan-600 dark:border-[#00F5FF] pb-1"
        >
          /EXPERIENCE
        </a>
        {["/skills", "/projects", "/contact"].map((i) => (
          <a
            key={i}
            href={`#${i.substring(1)}`}
            className="text-slate-500 dark:text-white/40 hover:text-slate-900 dark:hover:text-white transition-colors pb-1"
          >
            {i}
          </a>
        ))}
      ...
      <div className="flex items-center gap-4 md:gap-6">
        ...
        <button className="bg-cyan-600 text-white hover:bg-cyan-700 dark:bg-[#00F5FF] dark:text-black dark:hover:bg-white font-mono px-4 md:px-5 py-2 text-xs font-bold transition-colors">
          Resume.pdf
        </button>
      </div>
    </nav>
